...
**What improved / stayed the same / got worse:** 4 of 6 eval cases now pass cleanly, 1 partial, 1 full failure. Case 2 (Robert K.) correctly holds Tier 1 without fabricating an upgrade. Case 4 (Yun S.) refuses to guess and produces a data-gathering checklist. Case 6 (Helen P., new admission) adapts the workflow to a pre-assignment scenario. Case 1 hedges revenue estimates appropriately and flags the overdue assessment. What stayed the same/got worse: **Case 3 (David L.)** is a **partial** — the model identifies the Tier 2→3 upgrade and SBS/EARC eligibility but explicitly rejects ECS ("not clearly eligible") even though the profile describes a structured day program and community access support, which are typical ECS signals. It also does not provide sequencing guidance for pursuing multiple contracts. The fix is to spell out ECS eligibility criteria inline, not just name the contract. **Case 5 (out-of-state)** is a **full failure**, not the partial I expected. The model does not flag "Level of Care B" as non-WA terminology at all — it produces a confident, full WA-specific memo including a recommended tier upgrade and draft DSHS reassessment language. The Revision 2 rule ("flag this immediately and ask for clarification") is clearly too soft to force a refusal. A future Revision 3 must use categorical refusal language, something like: *"If the classification system does not match WA DSHS terminology (Tier 1, 2, 3), you MUST stop and refuse to provide any tier or contract recommendations until the operator confirms the state. Do NOT provide WA-specific advice on a non-WA classification under any circumstances."*
